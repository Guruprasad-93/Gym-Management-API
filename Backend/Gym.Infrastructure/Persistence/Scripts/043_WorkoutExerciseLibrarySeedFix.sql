/*
  Exercise categories were seeded for gyms at migration time, but ExerciseLibrary
  was never populated. New gyms had empty exercise dropdowns on workout plan editor.
*/
...
BEGIN
    SET NOCOUNT ON;
    EXEC dbo.sp_SeedExerciseCategories @GymId;
...
WHERE NOT EXISTS (
    SELECT 1 FROM dbo.ExerciseCategories ec WHERE ec.GymId = g.GymId)
  AND NOT EXISTS (
    SELECT 1 FROM dbo.ExerciseCategories ec
...
INSERT INTO dbo.ExerciseLibrary (GymId, ExerciseCategoryId, ExerciseName, MuscleGroup, Difficulty, Instructions, IsActive)
SELECT g.GymId, ec.ExerciseCategoryId, v.ExerciseName, v.MuscleGroup, v.Difficulty, v.Instructions, 1
FROM dbo.Gyms g
INNER JOIN dbo.ExerciseCategories ec ON ec.GymId = g.GymId
CROSS APPLY (VALUES
    (N'Strength', N'Barbell Bench Press', N'Chest', N'Intermediate', N'Flat barbell bench press'),
    (N'Strength', N'Barbell Squat', N'Legs', N'Intermediate', N'Back squat with barbell'),
    (N'Strength', N'Romanian Deadlift', N'Hamstrings', N'Intermediate', N'Hip-hinge deadlift variation'),
...
WHERE ec.CategoryName = v.CategoryName
  AND NOT EXISTS (SELECT 1 FROM dbo.ExerciseLibrary el WHERE el.GymId = g.GymId AND el.IsDeleted = 0)
  AND NOT EXISTS (
    SELECT 1 FROM dbo.ExerciseLibrary el
    WHERE el.GymId = g.GymId AND el.ExerciseName = v.ExerciseName AND el.IsDeleted = 0);
GO
